...
from .forms import SignUpForm, ChangePasswordForm, BookingForm, BookingFormAdmin, VehicleForm, DriverForm, CombinedSelectionForm
...
def view_students_for_booking(request, booking_id):
    booking = get_object_or_404(Booking, pk=booking_id)
    students = booking.students.all()  # Assuming a ManyToMany relationship named `students`
    return render(request, 'view_students_for_booking.html', {
        'booking': booking,
        'students': students
    })
...
def students_pdf(request, booking_id):
    ...
    for student in students:
        lines.append(f'{student.first_name} {student.last_name} - {student.class_name}')
    ...
    filename = f'{booking.subject}_students.pdf'
...
def delete_booking(request, booking_id):
    ...
    if request.user == booking.teacher or request.user.is_superuser:
        booking.delete()
        messages.success(request, "Booking deleted!")
        return redirect('bookings-list')
    else:
        messages.error(request, "You aren't authorised to delete the booking")
        return redirect('bookings-list')
...
def add_booking(request):
    if request.method == "POST":
        form = BookingFormAdmin(request.POST) if request.user.is_superuser else BookingForm(request.POST)
        ...
        form = BookingFormAdmin() if request.user.is_superuser else BookingForm()
...
def select_students(request):
    if request.method == 'POST':
        form = CombinedSelectionForm(request.POST)
        if form.is_valid():
            selected_class = form.cleaned_data['selected_class']
            selected_students = form.cleaned_data['students']
            new_student_ids = list(selected_students.values_list('id', flat=True))

            # Get previously selected students from session
            previous_student_ids = request.session.get('selected_students', [])

            # Combine previous and new students, ensuring no duplicates
            all_student_ids = list(set(previous_student_ids + new_student_ids))

            # Save the combined list back to session
            request.session['selected_students'] = all_student_ids
            return redirect('review_students')
    else:
        form = CombinedSelectionForm()
    
    return render(request, 'select_students.html', {'form': form})
...
def review_students(request):
    ...
    if request.method == 'POST':
        ...
        if remove_student_id:
            student_ids.remove(int(remove_student_id))
            request.session['selected_students'] = student_ids
            return redirect('review_students')

    return render(request, 'review_students.html', {'students': selected_students})
...
def confirm_booking(request):
    if request.method == 'POST':
        ...
        if booking_id:
            ...
            student_ids = request.session.get('selected_students', [])
            selected_students = Student.objects.filter(id__in=student_ids)
            booking.students.set(selected_students)
            booking.save()
    ...
    student_ids = request.session.get('selected_students', [])
    selected_students = Student.objects.filter(id__in=student_ids)
    return render(request, 'confirm_booking.html', {'students': selected_students})
