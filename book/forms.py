...
class CombinedSelectionForm(forms.Form):
    selected_class = forms.ModelChoiceField(queryset=Class.objects.all(), label='Select Class')
    students = forms.ModelMultipleChoiceField(queryset=Student.objects.none(), widget=forms.CheckboxSelectMultiple, required=False)

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if 'selected_class' in self.data:
            try:
                class_id = int(self.data.get('selected_class'))
                self.fields['students'].queryset = Student.objects.filter(class_name_id=class_id)
